...
public class AddShampoo
{
	 ...
	 */
	public static void runAddShampoo(final long phno, final String cartName)
	{
		EventQueue.invokeLater(new Runnable()
		{
			public void run()
			{
				try
				{
					AddShampoo window = new AddShampoo(phno, cartName);
	 ...
	 * @throws SQLException 
	 */
	public AddShampoo(long phno, String cartName)
	{
	...
	private void initialize(final long phno, final String cartName)
	{
		...
		try
		{
			...
			ResultSet rs = st.executeQuery("select name,brand from items where type='Shampoo'");
			int i=0;
			while(rs.next())  
		...
		Products.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent arg0)
			{
				try
				{
					...
					final ResultSet rs = st.executeQuery("select qty,code,price from items where type='Shampoo' and brand = \"" +arr[0]+"\" and name=\""+arr[1]+"\"; ");
					int i=0;
					while(rs.next())
					{
						i = rs.getInt(1);
					...
					btnBuy.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent arg0) {
							try
							{
								...
								ResultSet rs = st.executeQuery("select code, price from items where type='Shampoo' and brand = \""+arr[0]+"\" and name = \""+arr[1]+"\";");
								while(rs.next())
								{
									code = rs.getInt(1);
									price = rs.getFloat(2);
