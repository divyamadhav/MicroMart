...
public class AddTea
{
	 ...
	 */
	public static void runAddTea(final long phno, final String cartName)
	{
		EventQueue.invokeLater(new Runnable()
		{
			public void run()
			{
				try
				{
					AddTea window = new AddTea(phno, cartName);
	 ...
	 * @throws SQLException 
	 */
	public AddTea(long phno, String cartName)
	{
	...
	private void initialize(final long phno, final String cartName)
	{
		...
		try
		{
			...
			ResultSet rs = st.executeQuery("select name,brand from items where type='Tea'");
			int i=0;
			while(rs.next())  
...
/*		String tea = (String) Products.getItemAt(Products.getSelectedIndex());
		final String arr[] = tea.split(" - ");
		...
		Products.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent arg0)
			{
				try
				{
					...
					final ResultSet rs = st.executeQuery("select qty,code,price from items where type='Tea' and brand = \"" +arr[0]+"\" and name=\""+arr[1]+"\"; ");
					int i=0;
					while(rs.next())
					{
						i = rs.getInt(1);
					...
					btnBuy.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent arg0) {
							try
							{
								...
								ResultSet rs = st.executeQuery("select code, price from items where type='Tea' and brand = \""+arr[0]+"\" and name = \""+arr[1]+"\";");
								while(rs.next())
								{
									code = rs.getInt(1);
									price = rs.getFloat(2);
...
//								int q = getInt((String) Quantity.getItemAt(Quantity.getSelectedIndex()));
								int q=0;
		...
		btnBuy.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try
				{
					...
					st.executeUpdate("insert into cart values (\"Mycart\",11543210,"+getString(code)+","+getString(q)+","+q*price+");");
				}
				catch (SQLException e1)
				{
					e1.printStackTrace();
